...
export interface FlutterExportOptions {
  /** Generated widget class name. */
  className?: string;
  /** Path for the generated widget file. */
  widgetFilePath?: string;
  /** App title used by the optional scaffold. */
  appTitle?: string;
  /** Flutter package name used by the optional scaffold. */
  packageName?: string;
}

const DEFAULT_CLASS_NAME = "GeneratedJsonRenderView";
const DEFAULT_WIDGET_FILE_PATH = "lib/generated_json_render.dart";
const DEFAULT_APP_TITLE = "json-render Flutter Demo";
const DEFAULT_PACKAGE_NAME = "json_render_flutter_demo";

function getClassName(options: FlutterExportOptions): string {
  return options.className ?? DEFAULT_CLASS_NAME;
}

function getWidgetFilePath(options: FlutterExportOptions): string {
  return options.widgetFilePath ?? DEFAULT_WIDGET_FILE_PATH;
}

function getWidgetImportPath(widgetFilePath: string): string {
  return widgetFilePath.startsWith("lib/")
    ? widgetFilePath.slice(4)
    : widgetFilePath;
}

function embedSpecJson(spec: Spec): string {
  return `r'''${JSON.stringify(spec, null, 2)}'''`;
}

function generateFlutterWidgetSource(
  spec: Spec,
  options: FlutterExportOptions = {},
): string {
  const className = getClassName(options);
  const embeddedSpecJson = embedSpecJson(spec);
...
const String _generatedSpecJson = ${embeddedSpecJson};
...
export function generateFlutterWidgetFile(
  spec: Spec,
  options: FlutterExportOptions = {},
): GeneratedFile {
  return {
    path: getWidgetFilePath(options),
    content: generateFlutterWidgetSource(spec, options),
  };
}
...
export function generateFlutterProject(
  ...
  const className = getClassName(options);
  const widgetFilePath = getWidgetFilePath(options);
  const appTitle = options.appTitle ?? DEFAULT_APP_TITLE;
...
description: Generated Flutter app from a json-render spec
publish_to: "none"
...
import '${getWidgetImportPath(widgetFilePath)}';
...
`,
    ...
    generateFlutterWidgetFile(spec, options),
  ];
}
